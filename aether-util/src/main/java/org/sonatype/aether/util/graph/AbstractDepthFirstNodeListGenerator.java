...
/**
 * Abstract base class for depth first dependency tree traversers. Subclasses of this visitor will visit each node exactly once
 * regardless how many paths within the dependency graph lead to the node such that the resulting node sequence is free of
 * duplicates.<br/> Actual vertex ordering (preorder, inorder, postorder) needs to be defined by subclasses through appropriate
 * implementations for {@link #visitEnter(org.sonatype.aether.graph.DependencyNode)} and {@link
...
public abstract class AbstractDepthFirstNodeListGenerator
  ...
  /**
   * Gets a class path by concatenating the artifact files of the visited dependency nodes. Nodes with unresolved artifacts are
