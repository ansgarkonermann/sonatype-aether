...
class AsyncRepositoryConnector
  ...
  /**
   * Create an {@link org.sonatype.aether.connector.async.AsyncRepositoryConnector} instance which connect to the {@link
   * RemoteRepository}
   *
  ...
  /**
   * Create a {@link FileLockCompanion} containing a reference to a temporary {@link File} used when downloading a remote file.
   * If a local and incomplete version of a file is available, use that file and resume bytes downloading. To prevent multiple
   * process trying to resume the same file, a {@link FileLock} companion to the tmeporary file is created and used to prevent
   * concurrency issue.
   *
  ...
  /**
   * Create a temporary file used to lock ({@link FileLock}) an associated incomplete file {@link File}. The {@link FileLock}'s
   * name is derived from the original file, appending ".lock" at the end. Usually this method gets executed when a download
   * fail to complete because the JVM goes down. In that case we resume the incomplete download and to prevent multiple process
   * to work on the same file, we use a dedicated {@link FileLock}.
   *
